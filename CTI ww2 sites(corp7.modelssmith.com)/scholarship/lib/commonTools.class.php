...
//  Begin common functions  
  ...
  public static function logMessage( $text ){
    file_put_contents('/tmp/scholarship.log',Date('Y-m-d H:i:s ').$text."\n",FILE_APPEND);
    return;
  }
  ...
  public static function csvMergeFileFromAssociativeArray( $array ) {
    // iterate through associative array and get fields and values
    // assumption: associative array is not nested
    $field_names = array( ); // to keep track of field names

    $content = '';
    foreach($array as $field => $value){
      $field = preg_replace('/[, ]+/','_',$field);
      $field = substr($field,0,31); 
      if(array_key_exists($field,$field_names)){
        for($i = 1; $i < 10; $i++){
          $field = substr($field,0,30) . $i;
          if(!array_key_exists($field,$field_names)){
            break;
          }
        }
  ...
  public static function PdfFromArray( $array, $title ){
   // Create a somewhat nicely formmatted text from the associative array

    $html = '<html><body>';
    foreach($array as $key => $value) {
      $ascii = iconv('UTF-8', 'ASCII//TRANSLIT', $value); // translate UTF8 to ASCII
      $html .= "<p><b>$key</b>:\n";
      $html .= "$ascii<br>&nbsp;</p>\n";
    }
    $html .= '</body></html>';
    ...
    // run() outputs file and stops processing when done
    return;
  }
  ...
  public static function createParamName( $string, $chars = 30 ){
    // Example: "1.	How often will you be willing to travel to a course location outside of your city/country of residence?" --> 
