...
class adminActions extends sfActions
{
  ...
  */
  ...
  public function executeSetPasswordToWelcome( sfWebRequest $request)
  {
    $id = $request->getParameter( 'id' );
    $profile = ProfilePeer::retrieveByPk( $id );
    if(isset($profile)){
      $profile->setPasswordToWelcome();
    }

    $this->redirect('admin/editUser?id='.$id.'&changed=Password+is+now+set+to+welcome');
  }

  public function executeSetToUK( sfWebRequest $request)
  {
    $id = $request->getParameter( 'id' );
    $profile = ProfilePeer::retrieveByPk( $id );
    if(isset($profile)){
    ...
    $this->redirect('admin/editUser?id='.$id.'&changed=Country+is+now+set+to+United+Kingdom');
  }

// Ticket #559: This method is added to provide option to set the country to UK.
  public function executeSetToUS( sfWebRequest $request)
  {
    $id = $request->getParameter( 'id' );
    $profile = ProfilePeer::retrieveByPk( $id );
    if(isset($profile)){
    ...
    $this->redirect('admin/editUser?id='.$id.'&changed=Country+is+now+set+to+USA');
  }
  ...
  public function executeEditUser(sfWebRequest $request)
  {
    ...
    if($id > 0){
      ...
      $this->has_taken  = $profile->hasTakenInTheBones() ? 'Yes' : 'No'; // this checks FileMaker immediately
      ...
      $this->convertedToCoach = $profile->getCredentialUpdated();
  ...
  public function executeContactCoaches(sfWebRequest $request)
  {
    ...
    $profile = $this->getUser()->getProfile(); // administrator

    if(isset($profile)){
      $from_email =  $profile->getEmail();
    }
    else {
      $from_email = 'registration@thecoaches.com';
    }
  ...
  public function executeDoListOfUsers(sfWebRequest $request)
  {
    ...
    $countryGroup = $request->getParameter( 'countryGroup' );

    $countries = array('US-Canada-ITB','UK-ITB','US-Canada-Synergy','UK-Synergy');
    ...
    foreach($profiles as $p){
      if($p->getCountryGroup() == $countryGroup){
        $name          = $p->getName();
        $email         = $p->getEmail();
...
CSV;
    ...
    $this->getResponse()->setHttpHeader('Content-Disposition', 'attachment;filename=Coach-Match-'.$countries[$countryGroup].'-User-List.csv', TRUE);
