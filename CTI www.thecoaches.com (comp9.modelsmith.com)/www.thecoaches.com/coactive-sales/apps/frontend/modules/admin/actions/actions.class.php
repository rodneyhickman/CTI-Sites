  ...
  */
    ...
    if($audio_url != ''){
      ...
      foreach ($request->getFiles() as $uploadedFile) {
        ...
        if($index == 0){ // first file
          $audio->setUrl( 'http://www.thecoaches.com/coactive-sales-class/uploads/'.$filename );
          $index++;
        } 
      }
      $audio->save();
  ...
  public function executeDocumentUpload(sfWebRequest $request)
  {
    ...
    foreach ($request->getFiles() as $uploadedFile) {
      ...
      move_uploaded_file($uploadedFile["tmp_name"], sfConfig::get('sf_upload_dir').'/'.$filename);
      if($index == 0){ // first file
        $document->setUrl( 'http://www.thecoaches.com/coactive-sales-class/uploads/'.$filename );
        $index++;
      } 
    }
    $document->save();
