...
if($mode == 'lead'){
   // $result = mysql_query("select booking_link,call_time,start_date from event_calendar where course_type_id=120 and `event` like '%why co-active leadership%' order by fmid asc");
       $result = mysql_query("select event,fmid,booking_link,call_time,start_date from event_calendar where course_type_id=120 and TRIM(booking_link) <>'' and `event` like '%leadership%' and  not `event` like 'turkey%' order by start_date desc");
...
if($link == ''){
    $link = "https://attendee.gotowebinar.com/register/912364117452014338";
}
