...
if($mode == 'lead'){
   // $result = mysql_query("select booking_link,call_time,start_date from event_calendar where course_type_id=120 and `event` like '%why co-active leadership%' order by fmid asc");
    ...
    $el = 'webinar-fun-footer';
}
else if($mode == 'fund'){
    $result = mysql_query("select booking_link,call_time,start_date from event_calendar where course_type_id=120 and TRIM(booking_link) <>'' and `event` like '%coaching%' order by start_date asc");
    $el = 'sidebar-link-fun';
}
else { // mode==fun
