...
AND (location='MTree' OR location='Wildwood' OR location='Sequoia' OR location="WESTERBEKE") ORDER BY start_date ASC
EOS;
...
$html .= emit_registration( $NA_retreats, 
                            ...
                            'Northern California',
                            '<input type="hidden" name="itemnc" id="itemnc" value="a-6814^LP^Total Cost USD $11,900, Deposit USD $975 <br> Leadership Package Northern California^975.00^1^^^^^">',
                            'op1','op2'
);
...
html += '';
html += '<p>';
html += '       <select id="reg_option" onchange="switchDiv(this.value);">';
html += '<option value="----">--- Select an option ---</option>';
html += '<option value="online" SELECTED>Online Registration</option>';
html += '<option value="download">Download and print registration form</option>';
html += '     </select>';
html += '</p>';
html += '';
html += '<!-- start of forms -->';
html += '<div style="float:left; width:500px;">                        ';
html += '  <div id="download" style="display:none;">';
html += '';
html += '';
html += '';
html += '    <p>To download and print the Registration Form, please enter your name, phone and email address and press Submit</p>';
html += '    ';
html += '    <form action="/docs/form.html" method="POST" name="form1" id="form1" style="margin-bottom: 0;" onsubmit="javascript:return ValidateForm(this)">';
html += '';
html += '      <p> ';
html += '        <input type="hidden" name="recipient" value="leadership@thecoaches.com">';
html += '';
html += '        <input type="hidden" name="subject" value="Leadership Enrollment Download Form">';
html += '        <input type="hidden" name="redirect" value="/docs/leadership/leadership-download-ef.html">';
html += '        ';
html += '        <strong></strong><strong>Name:</strong> ';
html += '        <input type="text" name="name" size="50" maxlength="50">';
html += '        <br>';
html += '        <strong>Email:</strong> ';
html += '        <input type="text" name="email" size="30" maxlength="30">';
html += '        <br>';
html += '        <strong></strong><strong>Phone ';
html += '          Number: </strong> ';
html += '        <input type="text" name="phone">';
html += '      </p>';
html += '';
html += '      <p style="margin-bottom: 0;"><strong> ';
html += '          <input type="submit" name="This completes leadership download  Form" value="Submit Form" tabindex="20">';
html += '        </strong><strong> ';
html += '          <input type="reset" value="Clear Form" tabindex="21" name="reset">';
html += '      </strong></p>';
html += '';
html += '    </form>';
...
html += '';
html += '          <tr>';
html += '            <td colspan="2"><b>Deposit:</b> The Deposit of $975 (US programs) or &euro;975 (European programs) is payable at time of registration.<br />';
...
html += '                <input type="radio" name="Payment Option" value="in full" checked>	In Full: $11,900 (US programs) or &euro;11,500. (European programs) due 3 weeks prior to the program start date</p>';
html += '              <p  style="text-indent:-17px; padding-left:17px;">';
html += '                <input type="radio" name="Payment Option" value="quarterly">	Quarterly: $2,000 (US programs) or &euro;1,900(European programs) due prior to the first retreat ';
html += '                and in 3 additional installments of $2,975 or &euro;2,875 3 weeks prior to each retreat start date</p>';
html += '              <p  style="text-indent:-17px; padding-left:17px;">';
html += '                <input type="radio" name="Payment Option" value="monthly">	Monthly: approximately $910.00 (US programs) or approximately &euro;877.00 (European programs) per month for 12 months, beginning 2 months prior to Retreat 1 (25% of the total program fees must be paid 3 weeks prior to each retreat start date)</p>';
html += '            </td>';
html += '          </tr>';
html += '';
html += '          <tr>';
...
html += '<p>&nbsp;</p>';
html += '';
html += '<input id="op31" name="op31" type="checkbox" value="Agreed to policies"> Yes, I have read and agree to <a class="pricingdetails" href="/_temp/policy_leadership.html" rel="lyteframe" rev="width: 800px; height: 500px; scrolling: auto;" title="CTI Policies">CTI Policies</a></p>';
html += '        ';
html += '';
html += '';
html += '';
