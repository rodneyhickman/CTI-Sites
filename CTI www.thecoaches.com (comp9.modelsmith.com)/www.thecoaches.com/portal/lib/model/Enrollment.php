...
// }]}
    ...
    file_put_contents('/tmp/portal_cancel.log',$info,FILE_APPEND);
 ...
 file_put_contents('/tmp/portal_cancel.log',$url."\n",FILE_APPEND);
    ...
    if($httpCode == 200){
      ...
      if(isset($result) && $result['result'] == 'ok'){
        ...
        if($new_course['customer code'] == $student->getFmid()){
          ...
          $this->save();
          file_put_contents('/tmp/portal_cancel.log',$json."\n",FILE_APPEND);
        }
        else {
          $msg = 'failed, system refused cancellation for unknown reason (Code 401)';
file_put_contents('/tmp/portal_cancel.log',$msg."\n",FILE_APPEND);
file_put_contents('/tmp/portal_cancel.log',$json."\n",FILE_APPEND);
        }
      }
      else {
