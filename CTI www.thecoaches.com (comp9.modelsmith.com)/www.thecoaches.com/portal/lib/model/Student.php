...
class Student extends BaseStudent
{
  ...
  public function refreshRecord(){
    ...
    if($httpCode == 200){
      // $result is JSON
      // if result = ok, then update transaction status in org record 
	  // Ticket #678: Parsing json data to remove hidden control characters using the regular expression /[[:cntrl:]]+/.
	  // json_decode failed due to the presence of some hidden control characters.  Referred from:
    ...
    if(isset($event)){
      ...
      if($enrollment){ // already registered...
        ...
        // Hence the template name is set.
          $this->setEventEligibilityStatusMsg("ALREADY_REGISTERED");
      ...
      if ($course_history == '') {
        ...
        // Hence the template name is set.
        $this->setEventEligibilityStatusMsg("ERROR_OCCURED");
        return false;
      } else {
//      Ticket # 552 - Validating the course.
          $course_type_id = $event->getCourseTypeId();
          if (!$this->validateCourse($course_type_id)) {
//            "You can only register to assist for courses you have completed"
              $this->setEventEligibilityStatusMsg("COURSE_NOT_TAKEN");
              return false;
          }
      }
      // now check if student is eligible
      $name = $event->getName();
      // Ticket # 552 - Removed the condition checking Five year Limit.
//      $course_type_id = $event->getCourseTypeId();
...
//        // Hence the template name is set.
//        $this->setEventEligibilityStatusMsg("NOT_WITHIN_FIVE_YEARS");
...
//      }
      ...
      if($level >= $min_level ){
        ...
        // Hence the template name is set.
        $this->setEventEligibilityStatusMsg("COURSE_LEVEL_NOT_MATCHED");
        return false;
      }
    }
    $this->setEventEligibilityStatusMsg("ERROR_OCCURED");
...
//  Ticket # 552 - Adding a new function validateCourse() to check the Student got completed the course.
//  Returns true if the Student has taken the course or false otherwise.
  public function validateCourse( $course_type_id ){
...
// - $results array should contain registered or waitlist
  ...
  public function waitlistAssist( $event_id ){
    //return array( 'msg' => 'fail'); // waitlisting temporarily disabled
