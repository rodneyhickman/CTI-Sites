...
class Event extends BaseEvent
{
  ...
  public function setTimes( $data ){
    if($this->getExtra7() !== $data){
      $this->setExtra7( $data );
    }
  }
  ...
  public function isAssistingShouldBeWaitlisted( $student ){
    ...
    // Ticket # 552 - Commented the following if condition.
...
//    }
    ...
    // Ticket # 552 - Commented the condition (num_of_days < 46 days)
...
//    }
    ...
    // Ticket # 552
    //90-60 days: priority should go to Leadership Program graduates and Certification graduates (CPCC's) only
    //Less than 60 days: assisting priority is open to everyone
    if ($num_of_days < 60) {
        return false;
    }
    ...
    if($num_of_days < 91){
        if ($num_of_days >= 60) {
            if ($student->getCPCCCertDate() != '' || $student->getLeadershipProgram() == 1) {
                return false;
            }            
        }
    }
