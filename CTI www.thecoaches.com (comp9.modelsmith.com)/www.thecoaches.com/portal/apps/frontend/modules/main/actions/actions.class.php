...
EOM;
 ...
 public function executeMySchedule3(sfWebRequest $request){
  ...
  public function executeUpdateEventsFromFM(sfWebRequest $request){
    ...
    //execute post
    $json     = curl_exec($ch);
    // Ticket 463:  The regular expression '/(?!\n)[\p{Cc}]/' is used to remove all invisible characters from the data returned by curl.  
    // These characters was the root cause to fail the json_decode.  
    // When json_decode failed, it returned null.
    // Please refer: http://uk.php.net/manual/en/regexp.reference.unicode.php
