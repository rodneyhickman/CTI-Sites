...
class PHPRestSQL {
    ...
    var $defaultPassword = "portal";
    var $defaultPasswordHex = '7f8380875a137a88875f7e3bc203e452d399aa1e83c83e2cc272b6162a40568c237e991de4b21990d481e1ec318fff596f2265bcf94255c640c00f9ff0a19107';
        ...
        if ($this->table && $this->uid) {
            if ($this->requestData) {
                ...
                if ($primary && count($primary) == count($this->uid)) { // update a row
                    ...
                    $sendEmail = false;
                    $sendNote = false; //used for coach and client notifications sent on match.
                    ...
                    if ($this->db->numRows($resource) > 0) { //this is a table with a user associated with it so create the user as well as the record
                        ...
                        $userdata = array();
                        $userfields = array("first_name", "last_name", "email", "username", "phone", "userType","sendemail","sendnotification");
                        foreach($userfields as $fld) {
                            if (isset($pairs[$fld])) {
                                ...
                                else if ($fld=="sendnotification") $sendNote = $pairs["sendnotification"];
                        ...
                        $this->logToFile("Invite User Type SQL: $sql");
                        $userResource = $this->db->executeStmt($sql);
                        $userRow = $this->db->row($userResource);
                        $userId = $userRow["user_id"];
                        $theUserType = $userRow["userType"];
                        $this->logToFile("Invite User Type A: $theUserType .");
                        ...
                        if (count($userdata)>0) {
                            ...
                            if ($resource) {
                                ...
                                //check for duplicate email / username here...
                        ...
                        if ($sendEmail && $sendEmail!="false") {
                            $errmsg = "";
                            $this->logToFile("Invite User Type B: $theUserType .");
                            $inviteType = "invite";
                            ...
                            if (!send_mail($this, $userId, $inviteType, $this->defaultPassword, $errmsg)) {
                                $this->respondWithSuccessObject('false',99, "Error sending mail: " . $errmsg);
                            } else {
                                $this->logToFile("Post Invite sent to $userId.");
                            }
                        }
                    }
            ...
            if ($this->requestData) {
               // prepare data for INSERT
                ...
                if ($this->table=="sessions" && $this->userType==1) {
                  ...
                  $resource = $this->db->executeStmt($sql);
                  if ($this->db->numRows($resource) > 0)
                    $this->respondWithSuccessObject('false',15, "Please assign a coach to this coachee before adding the session.");
                }

                //check for bill and pay rates for session entry
                if ($this->table=="sessions") {
                    if (!isset($pairs["bill_rate"])||!isset($pairs["pay_rate"])) {
                        $sql = "SELECT bill_rate, pay_rate FROM clients WHERE id=" . $pairs["client_id"];
                        $resource = $this->db->executeStmt($sql);
                ...
                //if ($this->table=="clients" || $this->table=="coachs" || $this->table=="organizations") {
                    ...
                    foreach($userfields as $fld) {
                        if (isset($pairs[$fld])) {
                            if ($fld=="email") $userdata["username"] = $pairs[$fld];
                                ...
                                unset($pairs[$fld]);
                            } else if ($fld =="sendnotification") {
                                $sendNote = $pairs["sendnotification"];
                                unset($pairs[$fld]);
                            } else $userdata[$fld] = $pairs[$fld];
                            unset($pairs[$fld]);
                        }
                    ...
                    if (!isset($userdata["password"])) $userdata["password"] = $this->defaultPasswordHex;
                ...
                if ($this->userType==3) $pairs["coach_id"] = $this->user_to_table_id("coachs", $this->userId);
                    ...
                    if ($pairs["coach_id"]==-1) $this->respondWithSuccessObject('false',"0","Error adding session: Coachee has not selected a coach.");
                }
                ...
                //TODO: check for error and on error delete the user record from above
                if (!$resource) {
                    $this->db->deleteRow('users', "id=$pairs[user_id]");
                    $this->respondWithSuccessObject("false",$this->db->getError(), "Error inserting record.  Not saved.");
                }
                ...
                if ($sendEmail && $sendEmail != "false") {
                    ...
                    if (!send_mail($this, $pairs["user_id"], $inviteType, $this->defaultPassword, $errmsg)) {
                        $this->db->deleteRow('users', "id=$pairs[user_id]");
                        $this->respondWithSuccessObject('false',99, "Error sending mail.  User not added: " . $errmsg);
                    } else {
                        $this->logToFile("Invite sent (Put).");
                    }
                }

                if ($sendNote && $sendNote != "false" && $this->table=="clients") { // && isset($pairs["coach_id"]) && $pairs["coach_id"]!="null") {
                    ...
                    if (!send_coach_client_notification_mail($this, $pairs["user_id"], $errmsg)) {
                        ...
                        $this->logToFile("Notifications Sent 2!");
                    }
