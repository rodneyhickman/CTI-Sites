...
function updateLogin(user, pw, newpass, callback) {
    //pw = hex_sha512(pw);
    api_call('POST',SERVER_URI + "/update",{"new_password":newpass,"username":user,"password":pw},callback);
}
...
function addCoach() { // values, optional sendemail=true, callback){
    ...
    //this is just in case... a bug was found where we sent coach_id.  I can repro, but this should fix it.
    delete values["coach_id"];
...
function api_call(type, url, data, callback) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data!=undefined && data.session_id!=undefined && data.session_id!=99) session_id = data.session_id;
