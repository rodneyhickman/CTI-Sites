...
class Forms
{
	...
	public function form($params=array(), $tagdata=NULL)
	{
		...
		if ( (isset($params['output_css']) === TRUE && $params['output_css'] != 'no') OR isset($this->EE->session->cache['Forms']['CSS']) === FALSE)
		{
		...
		if ( (isset($params['output_js']) === TRUE &&  $params['output_js'] != 'no') OR isset($this->EE->session->cache['Forms']['JS']) == FALSE)
		{
			//$css_js .= '<link rel="stylesheet" href="' . FORMS_THEME_URL . 'forms_base.css" type="text/css" media="print, projection, screen" />';
	...
	public function entries()
	{
		...
		// Form Name="" or ID?
		// -----------------------------------------
		if ($this->EE->TMPL->fetch_param('form_name') != FALSE OR $this->EE->TMPL->fetch_param('form_id') != FALSE)
		{
		...
		foreach ($query->result() as $count => $entry)
		{
			...
			foreach ($fields as $field)
			{
				...
				$vars = array();
				$vars[$prefix.'field'] = $this->EE->formsfields[ $field['field_type'] ]->output_data($field, $entry->$name ,'html');
				$vars[$prefix.'field_type'] = $field['field_type'];
				$vars[$prefix.'field_label'] = $field['title'];
				$vars[$prefix.'field_name'] = $field['url_title'];

				$inner .= $this->EE->TMPL->parse_variables_row($pair_data, $vars);
			}
