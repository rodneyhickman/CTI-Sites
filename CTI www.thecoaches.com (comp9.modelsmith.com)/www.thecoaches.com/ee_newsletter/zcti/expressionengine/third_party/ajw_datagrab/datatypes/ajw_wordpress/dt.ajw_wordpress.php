...
class Ajw_wordpress extends Datagrab_type {
	...
	function _fetch_xml( $xml ) {
		...
		foreach( $item_array as $i ) {
			...
			if( $this->settings["just_posts"] == "1" && $item["wp:post_type"] == "post" ) { 
				...
				// Fetch categories
				$item["categories"] = "";
				$categories = $this->_fetch_tags( $i, "category" );
	...
	function _fetch_tags( $xml, $tag ) {
		$count = preg_match_all( "|<$tag.*?>(.*?)</$tag>|is", $xml, $matches );
		if( $count == 0 ) {
			return array();
		}
		if( $count == 1 ) {
