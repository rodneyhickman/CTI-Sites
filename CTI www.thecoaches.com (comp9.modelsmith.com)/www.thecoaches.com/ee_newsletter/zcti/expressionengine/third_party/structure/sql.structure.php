...
class Sql_structure
{
	...
	function add_attributes($pages, $entry_id, $mode)
	{
		...
		{	
			...
			if ($has_children_class != 'no' && $page['rgt'] - $page['lft'] > 1 && ! array_key_exists('overview', $page))
			{
			...
			// first class
			if ($page['parent_id'] != 0 && array_key_exists($page['parent_id'], $pages) && ($page['lft']-1) == $pages[$page['parent_id']]['lft'])
		...
		if (count($top_array) > 0
			&& ! in_array('last', $pages[$first['entry_id']]['classes'])
			&& ! in_array('overview', $pages[$first['entry_id']]['classes']))
		{
	...
	*/
	...
	function get_uri()
	{
		return $this->EE->functions->remove_double_slashes('/'.implode('/',$this->EE->uri->segment_array()).'/');	
	}
