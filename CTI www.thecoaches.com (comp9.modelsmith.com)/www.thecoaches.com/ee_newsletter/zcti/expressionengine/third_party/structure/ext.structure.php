...
class Structure_ext {
	...
	function channel_module_create_pagination($ee_obj)
	{
		if ($this->_is_search() === FALSE && isset($this->EE->config->_global_vars['structure_pagination_segment']))
		{
			...
			{
				$ee_obj->offset = $this->EE->config->_global_vars['structure_pagination_page'];	
			}
	...
	*/
	...
	{
		$final_template = preg_replace_callback("({page_url_for:(\d{1,})})", array(&$this, '_parse_tag'), $final_template);

		return $final_template;
	}

	function _parse_tag($m)
	{
		$slug = array_key_exists($m[1], $this->site_pages['uris']) ? $this->EE->functions->remove_double_slashes($this->site_pages['url'] . $this->site_pages['uris'][$m[1]]) : NULL;
