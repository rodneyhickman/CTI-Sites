...
class ProfilePeer extends BaseProfilePeer
{
  ...
  public static function profilesBySurvey( $survey_id ){
    $profiles = null;

    // get one question from survey to count how many answersets there were
    $c = new Criteria();
    $c->add(QuestionPeer::SURVEY_ID, $survey_id);
    $c->add(QuestionPeer::IS_RANGE, 'yes'); // NOTE: this assumes that all surveys will have ranges. 
    $question = QuestionPeer::doSelectOne( $c );

    if(isset($question)){
      // get profiles for this question
      $c = new Criteria();
      $c->addJoin(ProfilePeer::ID, AnswerPeer::PROFILE_ID);
